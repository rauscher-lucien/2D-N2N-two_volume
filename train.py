...
class Trainer:
    ...
    def train(self):
        ...
        dataset_train = TwoVolumeDataset(self.train_data_dir, num_volumes=2, transform=transform_train)
        ...
        dataset_val = TwoVolumeDataset(self.val_data_dir, num_volumes=2, transform=val_transform)
        ...
        for epoch in range(st_epoch + 1, self.num_epoch + 1):
            ...
            for batch, data in enumerate(loader_train, 1):
                optimizer.zero_grad()
                input_slice, target_img = [x.to(self.device) for x in data]
            ...
            if epoch % self.val_freq == 0:
                ...
                with torch.no_grad():
                    for val_batch, val_data in enumerate(val_loader, 0):
                        val_input_img, val_target_img = [x.to(self.device) for x in val_data]
                        if len(input_slice.shape) == 3:
                            val_input_img = val_input_img[np.newaxis, ...]
                            val_target_img = val_target_img[np.newaxis, ...]
